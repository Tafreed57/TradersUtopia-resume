...
export function SideBarActions() {
	...
	const handleServerCreation = () => {
		if (!isAdmin) {
			alert("❌ Admin access required to create servers. Visit the dashboard to request admin access.");
			return;
		}
		onOpen("createServer");
	};
		...
		return (
			<div>
				<ActionTooltip align="center" side="right" label="Loading...">
					<button disabled className="group flex items-center cursor-not-allowed">
				...
				>
					<div className={`flex mx-3 h-[48px] w-[48px] rounded-[24px] group-hover:rounded-[16px] transition-all overflow-hidden items-center justify-center bg-background dark:bg-neutral-700 ${
						...
						{isAdmin ? (
							...
							<Lock size={25} className="group-hover:text-white transition text-red-500" />
						)}
					</div>
				</button>
			</ActionTooltip>
