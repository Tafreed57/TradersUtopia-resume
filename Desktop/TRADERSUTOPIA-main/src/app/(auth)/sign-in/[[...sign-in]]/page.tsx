...
export default function Page({ 
  ...
  useEffect(() => {
    ...
    const handleDocumentClick = (e: Event) => {
      ...
      console.log('👆 [OAUTH-DEBUG] Click detected on:', target.tagName, target.className, target.textContent?.slice(0, 50));
      
      // Cast a wider net for Google buttons
      const googleButton = target.closest('[data-provider="google"]') || 
                          target.closest('.cl-socialButtonsBlockButton__google') ||
                          target.closest('.cl-socialButtonsBlockButton') ||
                          target.closest('button') && (
                            target.closest('button')?.textContent?.includes('Google') ||
                            target.closest('button')?.textContent?.includes('Continue with Google') ||
                            target.closest('button')?.innerHTML?.includes('google') ||
                            target.closest('button')?.className?.includes('google')
                          ) ||
      ...
      if (googleButton) {
        ...
        const urlMonitor = setInterval(() => {
          ...
          console.log(`🔎 [OAUTH-DEBUG] Monitor check #${monitorCount}:`, currentUrl);
          ...
          if (currentUrl.includes('accounts.google.com')) {
            ...
            try {
              ...
              if (!url.searchParams.has('prompt')) {
                ...
                console.log('✅ [OAUTH-DEBUG] Redirecting with account selection:', url.toString());
                window.location.replace(url.toString());
              } else {
                console.log('✅ [OAUTH-DEBUG] URL already has prompt parameter');
              }
            } catch (e) {
              console.warn('❌ [OAUTH-DEBUG] Failed to modify current OAuth URL:', e);
            }
        ...
        setTimeout(() => {
          clearInterval(urlMonitor);
          console.log('⏰ [OAUTH-DEBUG] URL monitoring stopped after 10 seconds');
        }, 10000);
      }
    };
    ...
    const handleGoogleButtonClick = (e: Event) => {
      console.log('🎯 [OAUTH-DEBUG] Direct Google button click detected!', e.target);
    ...
    window.open = function(url?: string | URL, target?: string, features?: string) {
      if (typeof url === 'string' && url.includes('accounts.google.com/oauth')) {
        try {
          const urlObj = new URL(url);
          urlObj.searchParams.set('prompt', 'select_account');
          urlObj.searchParams.set('access_type', 'offline');
          url = urlObj.toString();
          console.log('✅ Modified Google OAuth popup URL to force account selection');
        } catch (e) {
        ...
        if (mutation.type === 'childList') {
          ...
          addedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              const links = element.querySelectorAll ? element.querySelectorAll('a[href*="accounts.google.com"]') : [];
              links.forEach((link: Element) => {
                ...
                if (anchor.href && anchor.href.includes('accounts.google.com/oauth')) {
                  try {
                    ...
                    console.log('✅ Modified Google OAuth link to force account selection');
                  } catch (e) {
    ...
    const debugPageElements = () => {
      console.log('🔍 [OAUTH-DEBUG] Scanning page for Google-related elements...');
      ...
      selectors.forEach(selector => {
        ...
        if (elements.length > 0) {
          console.log(`📍 [OAUTH-DEBUG] Found ${elements.length} elements with selector "${selector}":`, elements);
          elements.forEach((el, index) => {
            console.log(`  ${index + 1}. ${el.tagName} - Class: "${el.className}" - Text: "${el.textContent?.slice(0, 50)}" - HTML: "${el.innerHTML?.slice(0, 100)}"`);
          });
        }
      });
      
      // Check for OAuth links
      const existingLinks = document.querySelectorAll('a[href*="accounts.google.com"], a[href*="oauth"], a[href*="google"]');
      console.log('🔗 [OAUTH-DEBUG] Found OAuth/Google links:', existingLinks.length);
        ...
        if (anchor.href && anchor.href.includes('accounts.google.com/oauth')) {
          try {
            ...
            if (!url.searchParams.has('prompt')) {
              ...
              url.searchParams.set('access_type', 'offline');
              anchor.href = url.toString();
              console.log('✅ [OAUTH-DEBUG] Modified existing Google OAuth link to force account selection');
            }
          } catch (e) {
            console.warn('❌ [OAUTH-DEBUG] Failed to modify existing OAuth link:', e);
          }
        }
      });
    };
    ...
    const addDirectListeners = () => {
      const googleButtons = document.querySelectorAll('.cl-socialButtonsBlockButton__google, .cl-socialButtonsBlockButton');
      console.log(`🎯 [OAUTH-DEBUG] Adding direct listeners to ${googleButtons.length} Google buttons`);
      
      googleButtons.forEach((button, index) => {
        console.log(`  Button ${index + 1}:`, button.className, button.textContent);
