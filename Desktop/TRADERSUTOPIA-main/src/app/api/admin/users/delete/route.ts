...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🗑️ [ADMIN] Admin ${adminProfile.email} is deleting user ${targetProfile.email} (${userId})`);

    // Cancel Stripe subscription if exists
    if (targetProfile.stripeSessionId && targetProfile.subscriptionStatus === 'ACTIVE') {
      try {
        ...
        console.warn(`Failed to cancel Stripe subscription for user ${userId}:`, stripeError);
    ...
    console.log(`✅ [ADMIN] Successfully deleted user ${targetProfile.email} by admin ${adminProfile.email}`);
