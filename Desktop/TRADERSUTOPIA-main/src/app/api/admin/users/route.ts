...
async function getSubscriptionWithName(profile: any) {
  ...
  if (profile.stripeProductId) {
    ...
    if (productCache.has(profile.stripeProductId)) {
      ...
      try {
        ...
        console.warn(`Failed to fetch product name for ${profile.stripeProductId}:`, error);
      }
    }
  }
      ...
      if (subscriptions.data.length > 0) {
        ...
        if (latestSubscription.items.data.length > 0) {
          ...
          try {
            ...
            console.warn(`Failed to fetch price details for ${priceId}:`, priceError);
          }
        }
      }
    } catch (error) {
      console.warn(`Failed to fetch subscription details for customer ${profile.stripeCustomerId}:`, error);
    }
  }
  ...
  return {
    ...
    currentPeriodEnd: profile.subscriptionEnd?.toISOString() || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
    ...
    planName: planName,
    createdAt: profile.subscriptionStart?.toISOString() || profile.createdAt.toISOString(),
  };
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const enrichedUsers = await Promise.all(profiles.map(async (profile) => {
      try {
        ...
        try {
          ...
          console.warn(`Failed to fetch Clerk data for user ${profile.userId}:`, clerkError);
        }
        ...
        if (profile.stripeCustomerId) {
          try {
            ...
            console.warn(`Failed to fetch Stripe data for customer ${profile.stripeCustomerId}:`, stripeError);
          }
        }
        ...
        return {
          ...
          subscription: profile.subscriptionStatus !== 'FREE' ? await getSubscriptionWithName(profile) : undefined,
          stripeCustomer,
          clerkData,
        };
      } catch (error) {
    ...
    console.log(`📊 [ADMIN] Admin ${profile.email} fetched ${enrichedUsers.length} users`);
