...
export async function POST(request: NextRequest) {
  try {
    ...
    if (targetProfile.subscriptionStatus === 'FREE' || !targetProfile.stripeCustomerId) {
      return NextResponse.json({ error: 'User has no active subscription' }, { status: 400 });
    }

    console.log(`🚫 [ADMIN] Admin ${adminProfile.email} is cancelling subscription for ${targetProfile.email} (${userId})`);
    ...
    try {
      ...
      for (const subscription of subscriptions.data) {
        ...
        console.log(`💳 [ADMIN] Cancelled Stripe subscription ${subscription.id}`);
      }
    } catch (stripeError) {
      console.error(`Failed to cancel Stripe subscription for user ${userId}:`, stripeError);
      return NextResponse.json({ 
        error: 'Failed to cancel subscription in Stripe',
        message: 'Unable to process subscription cancellation. Please try again.'
      }, { status: 500 });
    }
    ...
    console.log(`🗄️ [ADMIN] Updated database subscription status to cancelled for user ${userId}`);

    console.log(`✅ [ADMIN] Successfully cancelled subscription for ${targetProfile.email} by admin ${adminProfile.email}`);
