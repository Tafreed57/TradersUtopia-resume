...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔄 [SYNC] Syncing subscription for: ${profile.stripeCustomerId}`);
    ...
    if (!subscriptionItem) {
      console.error('❌ [SYNC] No subscription items found:', activeSubscription.id);
    ...
    if (!periodStart || !periodEnd) {
      console.error('❌ [SYNC] Subscription missing period data at both levels:', activeSubscription.id);
      return NextResponse.json({ error: 'Subscription missing period information' }, { status: 400 });
    }

    if (!subscriptionItem.price.product) {
      console.error('❌ [SYNC] Subscription missing product data:', activeSubscription.id);
      return NextResponse.json({ error: 'Subscription missing product information' }, { status: 400 });
    }
    ...
    const stripeProductId = activeSubscription.items.data[0]?.price.product as string;
    ...
    if (isNaN(stripeStart.getTime()) || isNaN(stripeEnd.getTime())) {
      ...
      return NextResponse.json({ error: 'Invalid subscription dates from Stripe' }, { status: 400 });
    }

    console.log(`📅 [SYNC] Stripe dates: ${stripeStart.toISOString()} to ${stripeEnd.toISOString()}`);
    console.log(`📅 [SYNC] Database dates: ${profile.subscriptionStart ? profile.subscriptionStart.toISOString() : 'null'} to ${profile.subscriptionEnd ? profile.subscriptionEnd.toISOString() : 'null'}`);
