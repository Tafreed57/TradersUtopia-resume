...
import { validateInput, subscriptionCancelSchema, secureTextInput } from '@/lib/validation';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validationResult = await validateInput(subscriptionCancelSchema)(request);
    ...
    if (reason) {
      ...
      if (reasonCheck.threats.length) {
        console.warn(`🚨 [SECURITY] Suspicious cancellation reason: ${reasonCheck.threats.join(', ')}`);
        trackSuspiciousActivity(request, `CANCEL_REASON_THREATS_${reasonCheck.threats.join('_')}`);
        return NextResponse.json({ 
    ...
    console.log(`🔄 [CANCEL] Auto-renewal disabled for subscription: ${subscription.id}`);
    ...
    let periodEndTimestamp = (updatedSubscription as any).current_period_end || (subscription as any).current_period_end;
    ...
    if (!periodEndTimestamp && subscription.items?.data?.[0]) {
      ...
      console.log(`📋 [CANCEL] Using period end from subscription item: ${periodEndTimestamp}`);
    }
    
    // Validate timestamp before creating date
    if (!periodEndTimestamp || typeof periodEndTimestamp !== 'number') {
      console.error('❌ [CANCEL] Invalid period end timestamp:', periodEndTimestamp);
      return NextResponse.json({ 
    ...
    if (isNaN(periodEndDate.getTime())) {
      console.error('❌ [CANCEL] Invalid period end date created from timestamp:', periodEndTimestamp);
      return NextResponse.json({ 
        error: 'Invalid subscription period data',
    ...
    console.log(`📅 [CANCEL] Subscription will end at: ${periodEndDate.toISOString()}`);
    console.log(`📝 [CANCEL] Reason: ${sanitizedReason || 'No reason provided'}`);
    console.log(`👤 [CANCEL] User: ${profile.email} (${user.id})`);
    console.log(`📍 [CANCEL] IP: ${request.headers.get('x-forwarded-for') || 'unknown'}`);
