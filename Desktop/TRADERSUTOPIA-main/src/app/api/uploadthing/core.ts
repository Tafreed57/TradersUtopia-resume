...
const secureFileValidation = async (file: { name: string; size: number; type: string }, userId: string) => {
	// ✅ SECURITY: Rate limiting check
	const rateCheck = validateUploadRate(userId, 1);
	if (!rateCheck.allowed) {
		console.error(`🚨 [UPLOAD] Rate limit exceeded for user: ${userId} - ${rateCheck.reason}`);
	...
	try {
		...
		console.log(`✅ [UPLOAD] File security validation passed for user: ${userId}`, {
			fileName: file.name,
			fileSize: file.size,
			mimeType: file.type,
			scanTime: scanResult.scanTime
		...
		console.error(`❌ [UPLOAD] Virus scan error for user: ${userId}:`, scanError);
...
export const ourFileRouter = {
		...
		.middleware(async ({ files }) => {
			...
			if (!rateLimitResult.success) {
				console.error(`🚨 [UPLOAD] Rate limit exceeded for user: ${auth.userId}`);
	...
	messageFile: f(["image", "pdf"])
		.middleware(async ({ files }) => {
			...
			for (const file of files) {
				...
				if (file.type.startsWith('image/') && file.size > 8 * 1024 * 1024) { // 8MB limit for images
