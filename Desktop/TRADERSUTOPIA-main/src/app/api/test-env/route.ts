...
export async function GET() {
  ...
  const hasLivekitKeys = !!process.env.LIVEKIT_API_KEY && !!process.env.LIVEKIT_API_SECRET;
  ...
  const configurationCheck = {
    ...
    authentication: {
      ...
      CLERK_PUBLISHABLE_KEY: hasClerkPublishable ? "✅ Configured" : "❌ Missing",
    },
    media_services: {
  ...
  const allChecks = [
    hasStripeSecret, hasWebhookSecret, hasDatabaseUrl, 
    hasClerkSecret, hasClerkPublishable, hasLivekitKeys
  ];
