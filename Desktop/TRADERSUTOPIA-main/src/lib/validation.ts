...
export function validateInput<T>(schema: z.ZodSchema<T>) {
  return async (request: NextRequest): Promise<{ success: true; data: T } | { success: false; error: NextResponse }> => {
    try {
...
export const secureTextInput = (input: string): { clean: string; threats: string[] } => {
...
export const validateFileUpload = (file: { name: string; size: number; type: string }) => {
  const threats: string[] = [];
...
export const validateFileUploadAdvanced = (file: { 
  ...
  const suspiciousPatterns = [
    /virus/i, /malware/i, /trojan/i, /backdoor/i, /keylog/i,
...
export const analyzeFileContent = (content: Buffer | Uint8Array, mimeType: string): string[] => {
  const threats: string[] = [];
  ...
  if (expectedMagic) {
    ...
    const isValidMagic = expectedMagic.some(magic => fileHeader.startsWith(magic));
...
export const simulateVirusScan = async (file: { 
  ...
  const suspiciousKeywords = [
    'virus', 'malware', 'trojan', 'backdoor', 'keylog', 'rootkit',
    'ransomware', 'spyware', 'adware', 'worm', 'botnet'
  ];
...
export const validateUploadRate = (userId: string, fileCount: number = 1): { 
  allowed: boolean; 
  reason?: string; 
  resetTime?: Date;
} => {
