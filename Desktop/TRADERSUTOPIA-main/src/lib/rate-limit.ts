...
export const RATE_LIMITS = {
  ...
  SUBSCRIPTION_OPERATIONS: {
    ...
    message: 'Too many subscription operations. Please wait 30 minutes before trying again.',
  },
...
export const rateLimit = (config: RateLimitConfig, prefix: string = 'default') => {
  return async (request: NextRequest): Promise<{ success: true } | { success: false; error: NextResponse }> => {
    ...
    if (store.count >= config.maxRequests) {
      ...
      console.warn(`⚠️ [RATE LIMIT] ${prefix} - Key: ${key.slice(0, 20)}... - Limit exceeded`);
...
export const rateLimitAdmin = () => rateLimit(RATE_LIMITS.ADMIN_OPERATIONS, 'admin');
export const rateLimitPassword = () => rateLimit(RATE_LIMITS.PASSWORD_OPERATIONS, 'password');
export const rateLimit2FA = () => rateLimit(RATE_LIMITS.TWO_FACTOR_AUTH, '2fa');
export const rateLimitUpload = () => rateLimit(RATE_LIMITS.FILE_UPLOADS, 'upload');
export const rateLimitSubscription = () => rateLimit(RATE_LIMITS.SUBSCRIPTION_OPERATIONS, 'subscription');
export const rateLimitGeneral = () => rateLimit(RATE_LIMITS.GENERAL_API, 'general');
export const rateLimitDebug = () => rateLimit(RATE_LIMITS.DEBUG_ENDPOINTS, 'debug');
...
export const trackSuspiciousActivity = (request: NextRequest, reason: string) => {
  ...
  if (!activity || now - activity.lastSeen > 24 * 60 * 60 * 1000) { // 24 hours
  ...
  if (activity.count > 5) {
    console.error(`🚨 [SECURITY ALERT] High suspicious activity from ${key.slice(0, 30)}... - Reason: ${reason} - Count: ${activity.count}`);
  }
...
export const getRateLimitInfo = (request: NextRequest, prefix: string) => {
  ...
  const remaining = Math.max(0, RATE_LIMITS.GENERAL_API.maxRequests - store.count);
