...
import { validateInput, twoFactorCodeSchema, secureTextInput } from '@/lib/validation';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (codeCheck.threats.length) {
      console.warn(`🚨 [SECURITY] Suspicious 2FA disable code: ${codeCheck.threats.join(', ')}`);
    ...
    if (!profile.twoFactorEnabled || !profile.twoFactorSecret) {
      ...
      return NextResponse.json({ 
        ...
        message: 'Two-factor authentication is not currently enabled for this account'
      }, { status: 400 });
    }
    ...
    if (!isValid) {
      ...
      console.warn(`🚨 [2FA-DISABLE] Invalid code attempt for user: ${profile.email}`);
      return NextResponse.json({ 
        error: 'Invalid code',
    ...
    try {
      ...
      console.warn('⚠️ [2FA-DISABLE] Failed to create notification:', notificationError);
    }

    // ✅ SECURITY: Log successful 2FA disabling
    console.log(`🔒 [2FA-DISABLE] Two-factor authentication disabled for user: ${profile.email}`);
    console.log(`📍 [2FA-DISABLE] IP: ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    console.log(`🖥️ [2FA-DISABLE] User Agent: ${request.headers.get('user-agent')?.slice(0, 100) || 'unknown'}`);
    console.log(`⚠️ [2FA-DISABLE] Security Warning: Account now less secure without 2FA`);

    return NextResponse.json({
      success: true,
      message: '2FA has been successfully disabled.',
      warning: 'Your account is now less secure without two-factor authentication. Consider re-enabling it soon.'
    });
