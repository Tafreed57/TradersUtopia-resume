...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { percentOff, newMonthlyPrice, currentPrice, originalPrice, subscriptionId } = validatedData;

    // Use originalPrice if provided, otherwise fallback to currentPrice
    const basePrice = originalPrice || currentPrice;

    console.log(`🎯 [CREATE-COUPON] Creating ${percentOff}% off coupon for user: ${user.id}`);
    console.log(`📊 [CREATE-COUPON] Original: $${basePrice}, Current: $${currentPrice}, Target: $${newMonthlyPrice}`);
    ...
    const actualPercentOff = Math.round(((basePrice - newMonthlyPrice) / basePrice) * 100);
    
    console.log(`🔍 [CREATE-COUPON] Verification: Base: $${basePrice}, Target: $${newMonthlyPrice}`);
    console.log(`🔍 [CREATE-COUPON] Percentage: Frontend: ${percentOff}%, Calculated: ${actualPercentOff}%`);
    ...
    try {
      ...
      console.log(`✅ [CREATE-COUPON] Coupon created: ${coupon.id} with ${actualPercentOff}% discount`);
    } catch (stripeError) {
    ...
    try {
      updatedSubscription = await stripe.subscriptions.update(activeSubscription.id, {
        discounts: [{ coupon: coupon.id }],
        proration_behavior: 'create_prorations', // Handle mid-cycle changes
      });

      console.log(`✅ [CREATE-COUPON] Coupon applied to subscription: ${activeSubscription.id}`);
    } catch (stripeError) {
      console.error('❌ [CREATE-COUPON] Failed to apply coupon to subscription:', stripeError);
      
      // If we can't apply the coupon, delete it to avoid orphaned coupons
      try {
        await stripe.coupons.del(coupon.id);
        console.log(`🧹 [CREATE-COUPON] Cleaned up unused coupon: ${coupon.id}`);
      } catch (cleanupError) {
        console.error('❌ [CREATE-COUPON] Failed to cleanup coupon:', cleanupError);
      }
    ...
    const originalAmount = updatedSubscription.items.data[0]?.price?.unit_amount || 0;
    const discountedAmount = Math.round(originalAmount * (1 - actualPercentOff / 100));

    // Log successful operation
    console.log(`🎉 [CREATE-COUPON] Success! User: ${profile.email || user.id}`);
    console.log(`📊 [CREATE-COUPON] Discount: ${actualPercentOff}% off permanently`);
    console.log(`💰 [CREATE-COUPON] New amount: $${discountedAmount / 100}/month`);
    console.log(`🔗 [CREATE-COUPON] Coupon ID: ${coupon.id}`);
    console.log(`📅 [CREATE-COUPON] Applied to subscription: ${updatedSubscription.id}`);
