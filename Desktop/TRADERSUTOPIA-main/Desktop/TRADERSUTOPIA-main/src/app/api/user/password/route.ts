...
import { validateInput, passwordSchema, secureTextInput } from '@/lib/validation';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { action, currentPassword, newPassword, twoFactorCode, use2FA } = validationResult.data;
    ...
    if (currentPassword) {
      ...
      if (currentPasswordCheck.threats.length) {
        console.warn(`🚨 [SECURITY] Suspicious current password input: ${currentPasswordCheck.threats.join(', ')}`);
    ...
    if (newPasswordCheck.threats.length) {
      console.warn(`🚨 [SECURITY] Suspicious new password input: ${newPasswordCheck.threats.join(', ')}`);
      trackSuspiciousActivity(request, 'SUSPICIOUS_PASSWORD_INPUT');
    }
    ...
    if (action === 'change' && hasPassword) {
      ...
      if (use2FA && twoFactorCode) {
        ...
        if (twoFactorResult.valid) {
          ...
          console.log('✅ [PASSWORD] 2FA verification successful' + (twoFactorResult.usingBackupCode ? ' (backup code used)' : ''));
        } else {
        ...
        authenticationVerified = await verifyCurrentPassword(user.id, currentPassword);
        ...
        return NextResponse.json({ 
          ...
          message: 'Either current password or 2FA code is required to change password.'
        }, { status: 400 });
      }
    ...
    try {
      ...
      console.log(`🔐 [PASSWORD] Password successfully updated for user: ${user.id}`);
      console.log(`📝 [PASSWORD] Action: ${action}`);
      console.log(`🔒 [PASSWORD] Auth method: ${use2FA ? '2FA' : 'current password'}`);
      console.log(`📍 [PASSWORD] IP: ${request.headers.get('x-forwarded-for') || 'unknown'}`);
