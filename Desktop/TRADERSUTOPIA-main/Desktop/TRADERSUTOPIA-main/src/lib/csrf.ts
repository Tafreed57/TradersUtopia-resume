...
export const getCSRFStats = () => {
  ...
  return {
    ...
    oldestToken: Math.min(...Array.from(csrfTokenStore.values()).map(d => d.expires)),
    newestToken: Math.max(...Array.from(csrfTokenStore.values()).map(d => d.expires))
  };
};
...
export const csrfProtection = () => {
  return async (request: NextRequest): Promise<{ success: true } | { success: false; reason: string }> => {
...
export const strictCSRFValidation = async (request: NextRequest): Promise<boolean> => {
  ...
  const pathname = request.nextUrl.pathname;
  const isHighSecurity = highSecurityPaths.some(path => pathname.startsWith(path));
