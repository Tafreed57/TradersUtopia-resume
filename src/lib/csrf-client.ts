...
export async function fetchCsrfToken(): Promise<string> {
  try {
    ...
    if (DEBUG_CSRF)
      console.log(
        "✅ [CSRF] Token cached successfully, expires at:",
    ...
    throw new Error(
      "Failed to obtain security token. Please refresh the page.",
    );
  }
}
  ...
  try {
    ...
    if (response.status === 403) {
      ...
      if (errorData.error === "CSRF validation failed") {
        console.log(
          "🔄 [CSRF] Token invalid, fetching new token and retrying...",
        );
        clearCsrfToken();
