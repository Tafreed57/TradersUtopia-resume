...
async function checkWebhookConfiguration() {
  ...
  return NextResponse.json({
    ...
    test: 'check-config',
    results,
    status: results.hasStripeSecretKey && results.hasWebhookSecret ? 'CONFIGURED' : 'MISSING_CONFIG'
  });
}
...
export async function GET(request: NextRequest) {
  return NextResponse.json({
    ...
    usage: {
      method: 'POST',
      body: {
        action: 'check-config | test-endpoint | simulate-event | verify-stripe-webhooks',
        eventType: 'subscription.created | subscription.updated | subscription.deleted | payment.succeeded | payment.failed (for simulate-event)',
