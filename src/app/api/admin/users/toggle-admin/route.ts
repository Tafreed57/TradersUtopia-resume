...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!userId || typeof grantAdmin !== 'boolean') {
      return NextResponse.json({ error: 'User ID and grantAdmin boolean are required' }, { status: 400 });
    }
    ...
    console.log(`👑 [ADMIN] Admin ${adminProfile.email} is ${action} admin privileges ${grantAdmin ? 'to' : 'from'} ${targetProfile.email} (${userId})`);
    ...
    console.log(`🗄️ [ADMIN] Updated admin status for user ${userId} to ${grantAdmin}`);

    // Log this significant action
    console.log(`✅ [ADMIN] Successfully ${actionPast} admin privileges ${grantAdmin ? 'to' : 'from'} ${targetProfile.email} by admin ${adminProfile.email}`);
