...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `🧪 [Test Setup] Set profile ${testProfile.id} (${testProfile.userId}) to FREE status for testing`,
    );
...
export async function GET(request: NextRequest) {
  return NextResponse.json({
    ...
    usage:
      "POST to this endpoint to create a test scenario with mixed profile statuses",
    description:
      "Sets one profile to FREE status so you can test login sync functionality",
  });
}
