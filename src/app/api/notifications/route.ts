...
import { rateLimitNotification, trackSuspiciousActivity } from '@/lib/rate-limit';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validationResult = await validateInput(notificationActionSchema)(request);
    ...
    if (action === 'mark_read' && notificationId) {
      console.log(`📖 [NOTIFICATIONS] Marking notification as read: ${notificationId} for user: ${user.id}`);
    ...
    if (action === 'delete' && notificationId) {
      console.log(`🗑️ [NOTIFICATIONS] Delete action requested for notification: ${notificationId}`);
