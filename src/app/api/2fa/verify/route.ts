...
export async function POST(request: NextRequest) {
  try {
    ...
    if (codeCheck.threats.length) {
      console.warn(`🚨 [SECURITY] Suspicious 2FA code input: ${codeCheck.threats.join(', ')}`);
    ...
    if (!profile.twoFactorSecret) {
      ...
      return NextResponse.json({ 
        ...
        message: 'Two-factor authentication must be configured before verification'
      }, { status: 400 });
    }
    ...
    if (!isValid) {
      ...
      console.warn(`🚨 [2FA] Invalid verification attempt for user: ${profile.email}`);
      return NextResponse.json({ 
        error: 'Invalid code',
    ...
    console.log(`🔒 [2FA] Two-factor authentication enabled for user: ${profile.email}`);
    console.log(`📍 [2FA] IP: ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    console.log(`🖥️ [2FA] User Agent: ${request.headers.get('user-agent')?.slice(0, 100) || 'unknown'}`);
