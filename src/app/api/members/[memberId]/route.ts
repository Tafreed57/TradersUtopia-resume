...
export async function PATCH(req: NextRequest, { params }: { params: { memberId: string } }) {
	try {
		...
		console.log(`👥 [MEMBER] Role updated successfully by user: ${profile.email} (${profile.id})`);
		console.log(`📝 [MEMBER] Member ID: ${params.memberId}, New role: ${role}, Server: ${serverId}`);
		console.log(`📍 [MEMBER] IP: ${req.headers.get('x-forwarded-for') || 'unknown'}`);
...
export async function DELETE(req: NextRequest, { params }: { params: { memberId: string } }) {
	try {
		...
		console.log(`🥾 [MEMBER] Member kicked successfully by user: ${profile.email} (${profile.id})`);
		console.log(`📝 [MEMBER] Kicked member ID: ${params.memberId}, Server: ${serverId}`);
		console.log(`📍 [MEMBER] IP: ${req.headers.get('x-forwarded-for') || 'unknown'}`);
