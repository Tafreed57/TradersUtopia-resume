...
import { validateInput, autoRenewalSchema, secureTextInput } from '@/lib/validation';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!periodEndTimestamp && subscription.items?.data?.[0]) {
      ...
      console.log(`📋 [AUTORENEW] Using period end from subscription item: ${periodEndTimestamp}`);
    }
    
    // Validate timestamp before creating date
    if (!periodEndTimestamp || typeof periodEndTimestamp !== 'number') {
      console.error('❌ [AUTORENEW] Invalid period end timestamp:', periodEndTimestamp);
      return NextResponse.json({ 
    ...
    if (isNaN(periodEndDate.getTime())) {
      console.error('❌ [AUTORENEW] Invalid period end date created from timestamp:', periodEndTimestamp);
      return NextResponse.json({ 
        error: 'Invalid subscription period data',
    ...
    console.log(`🔄 [AUTORENEW] Auto-renewal ${autoRenew ? 'enabled' : 'disabled'} for user: ${profile.email}`);
    console.log(`📍 [AUTORENEW] IP: ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    console.log(`📅 [AUTORENEW] Subscription period ends: ${periodEndDate.toISOString()}`);
