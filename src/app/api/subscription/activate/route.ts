...
import { validateInput, subscriptionActivationSchema, secureTextInput } from '@/lib/validation';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validationResult = await validateInput(subscriptionActivationSchema)(request);
    ...
    if (paymentIdCheck.threats.length) {
      console.warn(`🚨 [SECURITY] Suspicious payment ID: ${paymentIdCheck.threats.join(', ')}`);
    ...
    console.log(`💳 [ACTIVATION] Processing payment activation for user: ${user.id}`);
    console.log(`🔢 [ACTIVATION] Payment ID: ${paymentId.substring(0, 10)}...`); // Only log first 10 chars
    console.log(`📍 [ACTIVATION] IP: ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    
    if (paymentData) {
      console.log(`💰 [ACTIVATION] Amount: ${paymentData.amount} ${paymentData.currency}`);
      console.log(`🏪 [ACTIVATION] Payment method: ${paymentData.paymentMethod}`);
    }
    ...
    try {
      ...
      console.error('❌ [ACTIVATION] Subscription activation error:', activationError);
    ...
    console.log(`✅ [ACTIVATION] Subscription activated successfully for user: ${user.id}`);
