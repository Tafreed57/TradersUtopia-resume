...
import { validateInput, revokeAccessSchema, secureTextInput } from '@/lib/validation';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (reasonCheck.threats.length) {
      console.warn(`🚨 [SECURITY] Suspicious revoke reason: ${reasonCheck.threats.join(', ')}`);
      trackSuspiciousActivity(request, `REVOKE_REASON_THREATS_${reasonCheck.threats.join('_')}`);
    ...
    console.log(`🚫 [REVOKE] Revoking access for user: ${user.emailAddresses[0]?.emailAddress || user.id}`);
    console.log(`📝 [REVOKE] Reason: ${reasonCheck.clean}`);
    console.log(`📍 [REVOKE] IP: ${request.headers.get('x-forwarded-for') || 'unknown'}`);
    console.log(`🖥️ [REVOKE] User Agent: ${request.headers.get('user-agent')?.slice(0, 100) || 'unknown'}`);
    ...
    console.log(`📅 [REVOKE] Subscription marked as expired at: ${updatedProfile.subscriptionEnd}`);
    console.log(`🔒 [REVOKE] Previous status: ${profile.subscriptionStatus} → New status: ${updatedProfile.subscriptionStatus}`);
