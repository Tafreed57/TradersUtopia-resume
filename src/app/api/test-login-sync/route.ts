...
async function performLoginSync(userEmail: string) {
	try {
		...
		if (activeProfile && freeProfiles.length > 0) {
			...
			console.log(`🎉 [Login Sync] Successfully synced ${successfulSyncs} profiles for ${userEmail}`);
			...
			const reason = activeProfile ? 'no FREE profiles to sync' : 'no ACTIVE profile found';
			console.log(`ℹ️ [Login Sync] No sync needed for ${userEmail} - ${reason}`);
			
			return {
				synced: false,
				reason: reason,
				totalProfiles: allProfiles.length,
				activeProfiles: allProfiles.filter(p => p.subscriptionStatus === 'ACTIVE').length,
				freeProfiles: allProfiles.filter(p => p.subscriptionStatus === 'FREE').length
			};
		}
	} catch (error) {
...
export async function POST(request: NextRequest) {
	try {
		...
		console.log(`🧪 [Test Login Sync] Testing login sync for user: ${userEmail}`);
...
export async function GET(request: NextRequest) {
	return NextResponse.json({
		...
		description: 'This simulates what happens when a user logs in and the system checks for profile sync'
	});
} 