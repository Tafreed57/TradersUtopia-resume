...
export function ServerSection({ section, server, role }: ServerSectionProps) {
  const params = useParams();
  const isOpen = useStore(state => state.isOpen);
  const onOpen = useStore(state => state.onOpen);

  const sectionWithChannels = section as Section & {
    channels: Channel[];
  };
