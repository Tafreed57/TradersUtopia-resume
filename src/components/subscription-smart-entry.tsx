...
export function SubscriptionSmartEntry({ customContent }: SubscriptionSmartEntryProps) {
  ...
  const checkSubscriptionStatus = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('✅ Subscription check result:', result.hasAccess ? 'Active subscription found' : 'No active subscription');
      } else {
  ...
  const buttonText = customContent || (
    !isSignedIn ? "Access Trading Platform" :
    hasSubscription ? "Access Trading Platform" : "Get Access - View Pricing"
  );
      ...
      {isProcessing ? (
        <div className="flex items-center gap-2">
          ...
          <span className="hidden sm:inline">
            {!isSignedIn ? 'Redirecting...' : 
             hasSubscription ? 'Accessing Trading Platform...' : 'Loading Pricing...'}
          </span>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="sm:hidden">
            {!isSignedIn ? "Access Platform" : 
             hasSubscription ? "Access Platform" : "View Pricing"}
          </span>
        </div>
      )}
    </Button>
