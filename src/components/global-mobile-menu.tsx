...
import { Menu, Home, DollarSign, LayoutDashboard, Server, Play, User, LogIn } from "lucide-react";
...
export function GlobalMobileMenu() {
  ...
  const { handleSmartNavigation: handleNavSmartRouting, isLoaded } = useSmartRouting({
  ...
  const handleNavigation = async (path: string, isServerLink = false, isScrollLink = false) => {
    setIsOpen(false);
  ...
  const navigationItems: Array<{
    ...
    { id: "free-videos", icon: Play, label: "Free Videos", path: "/free-videos" },
    { id: "pricing", icon: DollarSign, label: "Pricing", path: "/pricing" },
    { id: "dashboard", icon: LayoutDashboard, label: "Dashboard", path: "/dashboard" },
    { id: "servers", icon: Server, label: "Servers", path: "/dashboard", isServerLink: true },
  ];
        ...
        <div className="flex flex-col h-full pt-6">
          ...
          <div className="flex items-center gap-3 mb-8 pb-4 border-b border-gray-700/50">
            <div className="w-8 h-8 bg-gradient-to-br from-yellow-400 to-yellow-600 rounded-lg flex items-center justify-center shadow-lg">
              <Image src="/logo.png" alt="TradersUtopia" width={16} height={16} />
            </div>
            ...
            {navigationItems.map((item) => {
              ...
              return (
                <Button
                  ...
                  onClick={() => handleNavigation(item.path, item.isServerLink || false, item.isScrollLink || false)}
                >
